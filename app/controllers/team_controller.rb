class TeamController < ApplicationController
  ...
  def show
    ...
    @games = Game.where("home_team_id = ? AND games.id < 15942 AND games.id > 14932 AND postpone = ?", id, false)
        .or(Game.where("home_team_id = ? AND games.id < 13107 AND games.id > 10516 AND postpone = ?", id, false)
                .or(Game.where("home_team_id = ? AND games.id < 10070 AND games.id >= 9058 AND postpone = ?", id, false))).order('id DESC')
  end
  ...
  def filter
    ...
    if params[:team_id].present?
      @result = Game.where("games.id < 15942 AND games.id > 14932 AND postpone = ?", false)
                    .or(Game.where("games.id < 13107 AND games.id > 10516 AND postpone = ?", false)
                            .or(Game.where("games.id < 10070 AND games.id >= 9058 AND postpone = ?", false))).order('game_date DESC')
      puts "All"
      puts @result.length
