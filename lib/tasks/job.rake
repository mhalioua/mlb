...
namespace :job do
  ...
  task test: :environment do
    ...
    while i < 1700
      ...
      url = format('http://www.espn.com/mlb/schedule/_/date/%d%s%02d', game_day.year, game_day.month, game_day.day)
      puts i
      puts url
  ...
  task basketball_game: :environment do
    ...
    elements.each do |element|
      if element.children.length == 12
        home_runs += 1 if element.children[11].text.include?('Home Run')
      end
      next unless element['class'].include?('pbp_summary_bottom')
  ...
  task newworkbook_link: :environment do
    ...
    games.each_with_index do |game, _index|
      ...
      trs.each do |slice|
        ...
        away_score = slice.children[1].children[3].text.to_i
        home_score = slice.children[3].children[3].text.to_i
        next unless away_team.include?(away_team_data) && home_team.include?(home_team_data)
  ...
  task workbookHits: :environment do
    ...
    games.each do |game|
      ...
      if (game.id >= 311_826 && game.id <= 337_437) ||
         (game.id >= 363_736 && game.id <= 364_883) ||
         (game.id >= 366_037 && game.id <= 367_266) ||
         (game.id >= 368_491 && game.id <= 369_606) ||
         (game.id >= 370_722 && game.id <= 371_869) ||
         (game.id >= 373_023 && game.id <= 400_400)
      ...
      if (game.id >= 337_438 && game.id <= 363_735) ||
         (game.id >= 364_884 && game.id <= 366_036) ||
         (game.id >= 367_267 && game.id <= 368_490) ||
         (game.id >= 369_607 && game.id <= 370_721) ||
         (game.id >= 371_870 && game.id <= 373_022) ||
         (game.id >= 400_401)
  ...
  task byteam: :environment do
    ...
    team_abbrs.each do |home_team_abbr|
      (1..1).each do |type|
        ...
        elements = doc.css('tr')
        elements.each do |element|
          next if element.children.size < 8
          next if element.children[3].text == 'OPPONENT'
  ...
  task getTimes: :environment do
    ...
    games.each do |game|
      ...
      game_date += timeStadium[:time].to_i.hours
  ...
  task getML: :environment do
    ...
    while index_date <= Date.new(2018, 5, 3)
      ...
      elements.each do |element|
        ...
        home_name = @team[home_name] if @team[home_name]
        away_name = @team[away_name] if @team[away_name]
  ...
  task old: :environment do
    ...
    doc.css('.team-name a').each_with_index do |stat, index|
      puts stat.child.text[0...-3].to_s if index.odd?
    end
      ...
      away_batter.each do |batter|
        if batter.children[0].text == '2B:'
          ...
          b.each do |eachb|
            ...
            away_twob_total += eachb_index
          end
        end
        next unless batter.children[0].text == '3B:'
  ...
  task getWeather: :environment do
    ...
    games.each do |game|
      ...
      unless hourlyweathers[start_index].children[headers['Temp.']].children.size == 1
        first_temp = hourlyweathers[start_index].children[headers['Temp.']].children[1].children[0].text
      end
      unless hourlyweathers[start_index].children[headers['Dew Point']].children.size == 1
        first_dp = hourlyweathers[start_index].children[headers['Dew Point']].children[1].children[0].text
      end
      first_humid = hourlyweathers[start_index].children[headers['Humidity']].text
      unless hourlyweathers[start_index].children[headers['Pressure']].children.size == 1
        first_baro = hourlyweathers[start_index].children[headers['Pressure']].children[1].children[0].text
      end
      ...
      start_index += 1 if start_index < hourlyweathers.size - 1
      unless hourlyweathers[start_index].children[headers['Temp.']].children.size == 1
        second_temp = hourlyweathers[start_index].children[headers['Temp.']].children[1].children[0].text
      end
      unless hourlyweathers[start_index].children[headers['Dew Point']].children.size == 1
        second_dp = hourlyweathers[start_index].children[headers['Dew Point']].children[1].children[0].text
      end
      second_humid = hourlyweathers[start_index].children[headers['Humidity']].text
      unless hourlyweathers[start_index].children[headers['Pressure']].children.size == 1
        second_baro = hourlyweathers[start_index].children[headers['Pressure']].children[1].children[0].text
      end
      ...
      start_index += 1 if start_index < hourlyweathers.size - 1
      unless hourlyweathers[start_index].children[headers['Temp.']].children.size == 1
        third_temp = hourlyweathers[start_index].children[headers['Temp.']].children[1].children[0].text
      end
      unless hourlyweathers[start_index].children[headers['Dew Point']].children.size == 1
        third_dp = hourlyweathers[start_index].children[headers['Dew Point']].children[1].children[0].text
      end
      third_humid = hourlyweathers[start_index].children[headers['Humidity']].text
      unless hourlyweathers[start_index].children[headers['Pressure']].children.size == 1
        third_baro = hourlyweathers[start_index].children[headers['Pressure']].children[1].children[0].text
      end
      ...
      away_previous_games.each_with_index do |element, index|
        if element.away_pitcher_link == game.away_pitcher_link
          if index < 15
            ...
            away_pitcher_game_first_bb += element.away_pitcher_bb
            away_pitcher_game_first_h += element.away_pitcher_h
            away_pitcher_game_first_r += element.away_pitcher_r
            away_pitcher_game_opp_first_ip.push(element.home_pitcher_ip)
            away_pitcher_game_opp_first_bb += element.home_pitcher_bb
            away_pitcher_game_opp_first_h += element.home_pitcher_h
            away_pitcher_game_opp_first_r += element.home_pitcher_r
          else
            away_pitcher_game_second_ip.push(element.away_pitcher_ip)
            away_pitcher_game_second_bb += element.away_pitcher_bb
            away_pitcher_game_second_h += element.away_pitcher_h
            away_pitcher_game_second_r += element.away_pitcher_r
            away_pitcher_game_opp_second_ip.push(element.home_pitcher_ip)
            away_pitcher_game_opp_second_bb += element.home_pitcher_bb
            away_pitcher_game_opp_second_h += element.home_pitcher_h
            away_pitcher_game_opp_second_r += element.home_pitcher_r
          end
        else
          if index < 15
            away_pitcher_game_first_ip.push(element.home_pitcher_ip)
            away_pitcher_game_first_bb += element.home_pitcher_bb
            away_pitcher_game_first_h += element.home_pitcher_h
            away_pitcher_game_first_r += element.home_pitcher_r
            away_pitcher_game_opp_first_ip.push(element.away_pitcher_ip)
            away_pitcher_game_opp_first_bb += element.away_pitcher_bb
            away_pitcher_game_opp_first_h += element.away_pitcher_h
            away_pitcher_game_opp_first_r += element.away_pitcher_r
          else
            away_pitcher_game_second_ip.push(element.home_pitcher_ip)
            away_pitcher_game_second_bb += element.home_pitcher_bb
            away_pitcher_game_second_h += element.home_pitcher_h
            away_pitcher_game_second_r += element.home_pitcher_r
            away_pitcher_game_opp_second_ip.push(element.away_pitcher_ip)
            away_pitcher_game_opp_second_bb += element.away_pitcher_bb
            away_pitcher_game_opp_second_h += element.away_pitcher_h
            away_pitcher_game_opp_second_r += element.away_pitcher_r
          end
        end
      end
      ...
      home_previous_games.each_with_index do |element, index|
        if element.away_pitcher_link == game.home_pitcher_link
          if index < 15
            ...
            home_pitcher_game_first_bb += element.away_pitcher_bb
            home_pitcher_game_first_h += element.away_pitcher_h
            home_pitcher_game_first_r += element.away_pitcher_r
            home_pitcher_game_opp_first_ip.push(element.home_pitcher_ip)
            home_pitcher_game_opp_first_bb += element.home_pitcher_bb
            home_pitcher_game_opp_first_h += element.home_pitcher_h
            home_pitcher_game_opp_first_r += element.home_pitcher_r
          else
            home_pitcher_game_second_ip.push(element.away_pitcher_ip)
            home_pitcher_game_second_bb += element.away_pitcher_bb
            home_pitcher_game_second_h += element.away_pitcher_h
            home_pitcher_game_second_r += element.away_pitcher_r
            home_pitcher_game_opp_second_ip.push(element.home_pitcher_ip)
            home_pitcher_game_opp_second_bb += element.home_pitcher_bb
            home_pitcher_game_opp_second_h += element.home_pitcher_h
            home_pitcher_game_opp_second_r += element.home_pitcher_r
          end
        else
          if index < 15
            home_pitcher_game_first_ip.push(element.home_pitcher_ip)
            home_pitcher_game_first_bb += element.home_pitcher_bb
            home_pitcher_game_first_h += element.home_pitcher_h
            home_pitcher_game_first_r += element.home_pitcher_r
            home_pitcher_game_opp_first_ip.push(element.away_pitcher_ip)
            home_pitcher_game_opp_first_bb += element.away_pitcher_bb
            home_pitcher_game_opp_first_h += element.away_pitcher_h
            home_pitcher_game_opp_first_r += element.away_pitcher_r
          else
            home_pitcher_game_second_ip.push(element.home_pitcher_ip)
            home_pitcher_game_second_bb += element.home_pitcher_bb
            home_pitcher_game_second_h += element.home_pitcher_h
            home_pitcher_game_second_r += element.home_pitcher_r
            home_pitcher_game_opp_second_ip.push(element.away_pitcher_ip)
            home_pitcher_game_opp_second_bb += element.away_pitcher_bb
            home_pitcher_game_opp_second_h += element.away_pitcher_h
            home_pitcher_game_opp_second_r += element.away_pitcher_r
          end
        end
      end
      game.update(
