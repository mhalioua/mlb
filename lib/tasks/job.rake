...
namespace :job do
  ...
  task update_weather_source: :environment do
    gameDay = GameDay.where("date = ?", Date.today).first
    ...
    games.each do |game|
      ...
      forecasts.each do |weather|
        ...
        Weathersource.create(
          ...
          caption: weather_time(weather.game.game_date, weather.hour),
          temp: weather.temp,
          dew: weather.dp,
          humid: weather.hum,
      ...
      forecasts.each_with_index do |weather, index|
        ...
        Weathersource.create(
          ...
          block_number: block_number,
          row_number: row_number,
          caption: "#{index + 1} hour",
          temp: "#{temp_min}-#{temp_max}",
          dew: "#{dew_min.round}-#{dew_max.round}" %><br><%= "#{(dew_min+1).round}-#{(dew_max-1).round}",
          humid: hum_min === hum_max ? hum_min : "#{hum_min}-#{hum_max}",
          pressure: "#{(pressure_min*100).round%100}-#{(pressure_max*100).round%100}",
          wind: "#{wind_min}-#{wind_max} ,#{weather.wind_dir}",
          all_stadium_total_average_one: result[:total_avg_1],
          all_stadium_total_average_two: result[:total_avg_2],
          all_stadium_total_hits_average: result[:total_hits_avg],
          all_stadium_home_runs_average: result[:home_runs_avg],
