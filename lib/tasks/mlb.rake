namespace :mlb do
  ...
  task update_batters: :environment do
    Season.where('year = 2021').map { |season| season.update_batters }
  end

  task update_pitchers: :environment do
    Season.where('year = 2021').map { |season| season.update_pitchers }
  end

  task create_games: :environment do
    Season.where('year <= 2021').map { |season| season.create_games }
  end
  ...
  task update_forecast: :environment do
    ...
    GameDay.tomorrow.update_forecast if time.hour > 18
  end

  task update_batters_scout: :environment do
    Season.where('year = 2021').map { |season| season.update_batters_scout }
  end

  task update_pitchers_scout: :environment do
    Season.where('year = 2021').map { |season| season.update_pitchers_scout }
  end
  ...
  task umpire: :environment do
    Season.where('year = 2021').map { |season| season.umpire }
  ...
  task basic: %i[create_season create_teams create_player]

  task daily: %i[create_player update_batters update_pitchers update_pitchers_scout update_batters_scout image_upload get_roof]

  task source: %i[create_matchups create_bullpen update_games pitcher_box_score batter_box_score]

  task history: %i[umpire update_transactions play_by_play prev_pitchers pitcher_informations]

  task weather: %i[update_forecast update_weather]

  task hourly: %i[update_games pitcher_box_score batter_box_score play_by_play]

  task ten: %i[create_matchups create_bullpen prev_pitchers pitcher_informations]
end
